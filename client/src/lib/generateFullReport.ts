...
export function generateFullCouncilReport(data: ReportInput): void {
  ...
  doc.text("VDTI Methodology v2.0 · Assessment period: 1 January – 31 March 2026", margin, 83);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Assessment period: January – March 2026 · VDTI Methodology v2.0", margin + 4, y + 5);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...SLATE);
  ...
  const methodText = [
    ...
    ``,
    `The methodology is size-normalised to ensure fair comparison between councils serving populations of 500 and those serving 50,000+. Scores are calculated using weighted indicators across the four pillars, with evidence gathered from council websites, published governance documents, statutory registers, and ONS geographic data.`,
    ``,
    `This is a full subscriber report. Unlike the public assessment (which caps at 95/100), this report includes the full 100-point scale, incorporating indicators that require direct council verification.`,
  ];
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...DARK);
  doc.text("Gold — £390/year (+ VAT)", margin + 4, y + 29);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...SLATE);
  doc.setFontSize(7.5);
  ...
  doc.setFontSize(6.5);
  doc.setTextColor(...SLATE);
  const disclaimer = `This is a sample full report produced by Council ClearSight to demonstrate the depth and quality of a subscriber assessment. All data is based on publicly available sources collected during the January–March 2026 assessment window. The VDTI methodology (v2.0) is published in full at councilclearsight.org.uk/methodology. Council ClearSight is an independent assessment service and is not affiliated with any local authority, government department, or regulatory body. © Council ClearSight 2026.`;
  ...
  // ─── Save ──────────────────────────────────────────────────────────────────
  const filename = `Council_ClearSight_FULL_Report_${council.name.replace(/[^a-zA-Z0-9]/g, "_")}_2026.pdf`;
  doc.save(filename);
}
